...
class GoldWorker(ctx:Context,p:WorkerParameters):Worker(ctx,p){
 override fun doWork():Result=try{
  ...
  val referenceStart=LocalTime.of(0,5)
  val referenceEnd=LocalTime.of(0,20)

  // Referans SADECE 00:05-00:20 penceresindeki ilk başarılı ölçümden oluşturulur.
  // Uygulama gün içinde ilk kez açılırsa o anki fiyat referans yapılmaz.
  val storedDay=sp.getString("day","")
  val inReferenceWindow=!now.toLocalTime().isBefore(referenceStart) && now.toLocalTime().isBefore(referenceEnd)
  if(storedDay!=today && inReferenceWindow){
   sp.edit()
    .putString("day",today)
    .putLong("xau0",java.lang.Double.doubleToRawLongBits(xau))
    .putLong("gram0",java.lang.Double.doubleToRawLongBits(gram))
    .putBoolean("xAlert",false).putBoolean("gAlert",false)
    .putString("referenceTime",now.format(DateTimeFormatter.ofPattern("dd.MM.yyyy HH:mm")))
    .apply()
  }

  val hasTodayReference=sp.getString("day","")==today
  if(hasTodayReference){
   val x0=java.lang.Double.longBitsToDouble(sp.getLong("xau0",java.lang.Double.doubleToRawLongBits(xau)))
   val g0=java.lang.Double.longBitsToDouble(sp.getLong("gram0",java.lang.Double.doubleToRawLongBits(gram)))
   val xp=(xau/x0-1)*100
   val gp=(gram/g0-1)*100
   val th=sp.getFloat("threshold",2f).toDouble()
   ...
   alarm("Ons Altın",xp,"$"+"%.2f".format(xau),"xAlert")
   alarm("Gram Altın",gp,"%.2f TL".format(gram),"gAlert")
   sp.edit().putString("xauPct","%+.2f%%".format(xp)).putString("gramPct","%+.2f%%".format(gp)).apply()
  }else{
   sp.edit().putString("xauPct","Referans bekleniyor (00:05)").putString("gramPct","Referans bekleniyor (00:05)").apply()
  }
  sp.edit().putString("xau","$"+"%.2f".format(xau)).putString("gram","%.2f TL".format(gram)).putString("last",now.format(DateTimeFormatter.ofPattern("dd.MM.yyyy HH:mm"))).apply()
  Result.success()
